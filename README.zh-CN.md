...
- 输出达到单次上限时保存已经流出的 assistant 内容和系统中断事件，并从稳定断点自动续写一次；截断的工具调用不会执行。
- Excel 预览不再重复加载行数据，在保留外层留白的同时铺满可用区域，并补齐空白网格、选择、列宽调整和复制能力。
- Session 任务列表默认收起，在标题栏显示进度、当前任务和动态运行状态。
